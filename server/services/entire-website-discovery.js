...
export const discoveryLimits = { pages: 100, depth: 4, timeoutMs: 10_000, responseBytes: 1024 * 1024, redirectLimit: 3, sitemapDepth: 5, runMs: 5 * 60_000, aggregateBytes: 25 * 1024 * 1024, requestSpacingMs: 1_000 };
...
export async function discoverWebsite(rootUrl, budget = new DiscoveryBudget()) {
  ...
  const queue = [{ url: rootUrl, depth: 0 }]; const crawled = new Set(); let rootFetched = false;
  while (queue.length && candidates.size < limits.pages) { budget.ensure(); const item = queue.shift(); if (crawled.has(item.url) || !robots.allowed(item.url)) continue; crawled.add(item.url); add(item.url, item.depth === 0 ? 'ROOT' : 'LINK', { depth: item.depth });
    try { const response = await resource(item.url, budget, /^(text\/html|application\/xhtml\+xml)/i); if (item.depth === 0) rootFetched = true; const canonical = extractCanonicalUrl(response.body, item.url); if (canonical) add(canonical, 'CANONICAL', { discoveredFrom: item.url }); if (item.depth < limits.depth) for (const link of extractHtmlLinks(response.body, item.url)) if (!crawled.has(link) && !discoveryExclusion(link) && robots.allowed(link)) queue.push({ url: link, depth: item.depth + 1 }); } catch { /* Individual crawl failures are reported during baseline if selected. */ }
  }
  const selected = [...candidates.values()].slice(0, limits.pages); const capped = candidates.size >= limits.pages || budget.capped; return { selected, ignored: [...ignored.entries()].map(([url, reason]) => ({ url, reason })), found: candidates.size, capped, robotsSitemaps: robots.sitemaps.length, qualifying: rootFetched && !capped, rootFetched };
}
...
export async function createEntireWebsiteTarget({ url, label = '', filterNoise = true }) {
  ...
  const scan = await scanRepository.create({ site_id: site.id, trigger: 'INITIAL', status: 'RUNNING', idempotency_key: `initial:${site.id}` , started_at: new Date().toISOString() }); const budget = new DiscoveryBudget();
  try { const outcome = await discoverWebsite(rootUrl, budget); let succeeded = 0; let failed = 0;
    ...
    const status = succeeded === 0 ? 'FAILED' : failed || outcome.capped ? 'PARTIAL' : 'SUCCEEDED'; site = await siteRepository.update(site.id, { state: succeeded ? 'ACTIVE' : 'ERRORED', last_scan_at: new Date().toISOString(), next_rediscovery_at: succeeded ? new Date(Date.now() + 86400 * 1000).toISOString() : null, last_error_code: succeeded ? null : 'INITIAL_BASELINE_FAILED', last_error_message: succeeded ? null : 'No page could be baselined during initial discovery.' }); await scanRepository.update(scan.id, { status, completed_at: new Date().toISOString(), pages_attempted: outcome.selected.length, pages_succeeded: succeeded, pages_failed: failed, discovery_summary: summary }); return presentSite(site, await pageRepository.listBySite(site.id), summary, false);
